...
from Deep3DFaceRecon_pytorch.util.preprocess import align_factors, align_img_fromfactors
...
out_dir = os.path.join(indir, "smooth")
...
for img_file, lm_file in zip(img_files, lm_files):
    ...
    t_, s_ = align_factors(im, lm, lm3d_std, target_size=target_size, rescale_factor=rescale_factor)
...
if sigma != 0:
    t1s = gaussian_filter1d(t1s, sigma=sigma, axis=0)
    t2s = gaussian_filter1d(t2s, sigma=sigma, axis=0)   
    ss = gaussian_filter1d(ss, sigma=sigma, axis=0)

    
    
for img_file, lm_file, t1_, t2_, s_ in zip(img_files, lm_files, t1s, t2s, ss):
    ...
    smooth_t = np.array([t1_, t2_])
    smooth_s = s_
    
    # align_img_fromfactors(img, lm, t, s, mask=None, target_size=224., rescale_factor=102.)
    # trans_params, img_new, lm_new, mask_new
    _, im_high, _, _= align_img_fromfactors(im, lm, smooth_t, smooth_s, target_size=target_size, rescale_factor=rescale_factor)
