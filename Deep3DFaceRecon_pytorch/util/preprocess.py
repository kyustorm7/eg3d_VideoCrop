...
def rough_values(img, lm, t, s, target_size=224., mask=None):
    w0, h0 = img.size
    w = (w0*s).astype(np.int32)
    h = (h0*s).astype(np.int32)
    left = (w/2 - target_size/2 + float((t[0] - w0/2)*s)) 
    up = (w/2 - target_size/2 + float((h0/2 - t[1])*s))
    
    return left + target_size/2, up + target_size/2
...
# utils for face reconstruction
def align_factors(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    """
    Return:
        transparams        --numpy.array  (raw_W, raw_H, scale, tx, ty)
        img_new            --PIL.Image  (target_size, target_size, 3)
...
def align_img_fromfactors(img, lm, t, s, mask=None, target_size=224., rescale_factor=102.):
    """
    Return:
        transparams        --numpy.array  (raw_W, raw_H, scale, tx, ty)
        img_new            --PIL.Image  (target_size, target_size, 3)
...
"""
def align_img_coord(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    Return:
        transparams        --numpy.array  (raw_W, raw_H, scale, tx, ty)
        img_new            --PIL.Image  (target_size, target_size, 3)
        lm_new             --numpy.array  (68, 2), y direction is opposite to v direction
    ...
    # processing the image
    # left, up = resize_n_crop_coords_1(img, lm, t, s, target_size=target_size, mask=mask)
    rough_x, rough_y= rough_values(img, lm, t, s, target_size=target_size, mask=mask)
    return rough_x, rough_y


def smoother_crop(img, smooth_x, smooth_y, target_size=224., mask=None):
    w0, h0 = img.size
    w = (w0*s).astype(np.int32)
    h = (h0*s).astype(np.int32)
    smooth_x = smooth_x.astype(np.int32)
    smooth_y = smooth_y.astype(np.int32)
    
    left = smooth_x - target_size/2
    up = smooth_y + target_size/2
